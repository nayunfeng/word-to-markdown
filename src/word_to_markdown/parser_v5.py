...
from .image_extractor_v2 import paragraph_image_rel_ids
...
def _parse_paragraph(paragraph):
    ...
    if text:
        ...
        if heading_level is not None:
            elements.append({'type': 'heading', 'level': heading_level, 'text': text})
        elif _is_list_paragraph(paragraph, style_name, text):
            clean_text = text.lstrip('•*-· ').strip()
            elements.append({'type': 'list', 'items': [clean_text]})
        else:
            elements.append({'type': 'paragraph', 'text': text})

    for rel_id in paragraph_image_rel_ids(paragraph):
        elements.append({'type': 'image', 'rel_id': rel_id})
...
def parse_docx(path):
    ...
    for block in iter_block_items(doc):
        if isinstance(block, Paragraph):
            elements.extend(_parse_paragraph(block))
